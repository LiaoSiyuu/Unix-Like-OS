#pragma once
#ifndef OS_BUFFER_MANAGER_H
#define OS_BUFFER_MANAGER_H
...
private:
    void _NotAvail_(Buf* bp);				/* 从自由队列中摘下指定的缓存控制块buf */

private:
    Buf _bFreeList;						/* 自由缓存队列控制块，定义有自由队列队头*/
    Buf _m_Buf[NBUF];					/* 缓存控制块数组 */
    Buf _bDevtab;                        /* 设备控制块，定义有设备队列队头 */
    char _Buffer[NBUF][BUFFER_SIZE];	    /* 缓冲区数组 */
